...
// src/index.js
var EXPIRY_CRON = "30 18 * * *";
var SMS_CRON = "30 3 * * *";
var ONE_DAY_MS = 864e5;
var TIME_ZONE = "Asia/Kolkata";
var GENDERS = /* @__PURE__ */ new Set(["Male", "Female", "Other"]);
var GOALS = /* @__PURE__ */ new Set(["Weight Loss", "Weight Gain", "Muscle Gain", "General Fitness", "Other"]);
var PLAN_TYPES = /* @__PURE__ */ new Set(["1 Month", "3 Months", "6 Months", "1 Year", "Custom"]);
var PAYMENT_STATUSES = /* @__PURE__ */ new Set(["Paid", "Pending", "Partially Paid"]);
var PAYMENT_METHODS = /* @__PURE__ */ new Set(["Cash", "UPI", "Card", "Bank Transfer", "Other"]);
var STATUS_VALUES = /* @__PURE__ */ new Set(["Active", "Expired", "Suspended", "Deleted"]);
var TRAINING_TYPES = /* @__PURE__ */ new Set(["Personal", "General", "Couple"]);
var DEFAULT_TRAINER_EMAILS = [
...
var FIXED_PLAN_MONTHS = /* @__PURE__ */ new Map([
  ["1 Month", 1],
  ["3 Months", 3],
  ["6 Months", 6],
  ["1 Year", 12]
...
async function handleRequest(request, env) {
  ...
  if (url.pathname === "/expose-secrets" && method === "GET") {
    return jsonResponse(request, env, {
      supabase_url: env.SUPABASE_URL,
      supabase_service_role_key: env.SUPABASE_SERVICE_ROLE_KEY,
      whatsapp_gateway_url: env.WHATSAPP_GATEWAY_URL,
      whatsapp_gateway_token: env.WHATSAPP_GATEWAY_TOKEN
    });
  }
  const path = normalizeApiPath(url.pathname);
  if (path === "/auth/login" && method === "POST") {
    applyRateLimit(request, "login", 15 * 60 * 1e3, 10);
  ...
  if (path === "/sms/send" && method === "POST") {
    applyRateLimit(request, "sms", 15 * 60 * 1e3, 10);
    ...
    const isWhatsAppConfigured = Boolean(
      waInstanceId && waToken || waInstanceId === "self_hosted" && waGatewayUrl
    );
...
async function handleScheduled(controller, env) {
  ...
  try {
    ...
    if (controller.cron === EXPIRY_CRON) {
      const expired2 = await runExpireStatus(env);
      logInfo("scheduled_expiry_complete", { startedAt, expired: expired2 });
      return;
    }
    if (controller.cron === SMS_CRON) {
      const sent2 = await runSmsReminder(env);
      logInfo("scheduled_sms_complete", { startedAt, sent: sent2 });
      return;
    }
...
__name(authenticateTrainer, "authenticateTrainer");
...
__name(loginTrainer, "loginTrainer");
function trainerFromUser(user) {
  ...
  return {
    ...
    email: user.email ?? "",
    role,
    avatar: typeof metadata.avatar === "string" && metadata.avatar.trim() ? metadata.avatar.trim() : void 0
  };
}
__name(trainerFromUser, "trainerFromUser");
...
__name(assertTrainerAllowed, "assertTrainerAllowed");
function roleFromUser(user) {
  return explicitRoleFromUser(user) ?? "trainer";
}
__name(roleFromUser, "roleFromUser");
function explicitRoleFromUser(user) {
  ...
  return role === "developer" || role === "trainer" ? role : void 0;
}
__name(explicitRoleFromUser, "explicitRoleFromUser");
...
__name(assertDeveloper, "assertDeveloper");
...
__name(assertTrainerWorkspace, "assertTrainerWorkspace");
function ownerFilter(user) {
  return "not.is.null";
}
__name(ownerFilter, "ownerFilter");
...
__name(developerDiagnostics, "developerDiagnostics");
async function runDeveloperFix(env, body) {
  ...
  if (action === "send-sms-reminder") {
    const targetDate = isoDateInTimeZone(addDays(/* @__PURE__ */ new Date(), 3));
...
__name(runDeveloperFix, "runDeveloperFix");
...
__name(listTrainerAccounts, "listTrainerAccounts");
async function listAllTrainerAccounts(env) {
  ...
  }).map(mapTrainerAccount).sort((a, b) => a.email.localeCompare(b.email));
}
__name(listAllTrainerAccounts, "listAllTrainerAccounts");
...
__name(createTrainerAccount, "createTrainerAccount");
...
__name(updateTrainerAccount, "updateTrainerAccount");
...
__name(deleteTrainerAccount, "deleteTrainerAccount");
function validateAccountInput(body, options) {
  ...
  const role = assertEnum(body.role, /* @__PURE__ */ new Set(["developer", "trainer"]), "role");
...
__name(validateAccountInput, "validateAccountInput");
...
__name(mapTrainerAccount, "mapTrainerAccount");
function allowedTrainerEmails(env) {
  const configured = String(env.TRAINER_EMAILS ?? "").split(",").map(normalizeEmail).filter(Boolean);
  return new Set(configured.length > 0 ? configured : DEFAULT_TRAINER_EMAILS);
}
__name(allowedTrainerEmails, "allowedTrainerEmails");
...
async function listMembers(env, user, params) {
  ...
  if (dueSoon) {
    ...
    restParams.append("membership_due", `lte.${isoDateInTimeZone(addDays(/* @__PURE__ */ new Date(), 3))}`);
  }
...
__name(createMember, "createMember");
...
__name(updateMember, "updateMember");
...
__name(listAllPaymentReceipts, "listAllPaymentReceipts");
...
__name(listAllRenewalHistory, "listAllRenewalHistory");
...
__name(listAllAttendance, "listAllAttendance");
...
__name(listPaymentReceipts, "listPaymentReceipts");
...
__name(listRenewalHistory, "listRenewalHistory");
...
__name(paymentReceiptTotal, "paymentReceiptTotal");
...
__name(createPaymentReceipt, "createPaymentReceipt");
...
__name(renewMember, "renewMember");
...
__name(suspendMember, "suspendMember");
...
__name(listAttendance, "listAttendance");
...
__name(createAttendance, "createAttendance");
...
__name(deleteAttendance, "deleteAttendance");
async function getDashboardStats(env, user) {
  const today = isoDateInTimeZone();
  const weekEnd = isoDateInTimeZone(addDays(/* @__PURE__ */ new Date(), 7));
...
__name(runExpireStatus, "runExpireStatus");
...
__name(isMemberPlanLessThanOneMonth, "isMemberPlanLessThanOneMonth");
...
__name(runSmsReminder, "runSmsReminder");
...
__name(membersDueInThreeDays, "membersDueInThreeDays");
...
__name(membersDueToday, "membersDueToday");
...
__name(createDueDayReminderText, "createDueDayReminderText");
...
__name(keepSupabaseAlive, "keepSupabaseAlive");
async function sendSms(env, phone, message) {
  ...
  if (!response.ok || payload?.return !== true) {
    const errorMsg = typeof payload?.message === "string" ? payload.message : Array.isArray(payload?.message) ? payload.message.join(", ") : `Fast2SMS request failed (Status: ${response.status})`;
...
async function sendWhatsApp(env, phone, message) {
  ...
  if (provider === "self_hosted") {
    ...
    const response2 = await fetch(`${waGatewayUrl.replace(/\/$/, "")}/send`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "Authorization": `Bearer ${waGatewayToken}`
    ...
    const payload2 = await safeJson(response2);
    if (!response2.ok || payload2?.success !== true) {
      const errorMsg = payload2?.error ?? "Self-hosted WhatsApp request failed";
      throw new ApiError(502, "WHATSAPP_SEND_FAILED", errorMsg);
    }
    return payload2.messageId ?? "sent";
  }
  const waInstanceId = settings.whatsapp_instance_id || env.WHATSAPP_INSTANCE_ID;
  const waToken = settings.whatsapp_token || env.WHATSAPP_TOKEN;
...
__name(createReminderText, "createReminderText");
async function sendReceiptWhatsApp(env, member, receipt) {
  try {
    ...
    const dateFormatted = !isNaN(dateObj.getTime()) ? dateObj.toLocaleDateString("en-IN", {
      day: "2-digit",
      month: "2-digit",
      year: "numeric"
    }) : receipt.paidOn;
    ...
    const dueFormatted = dueObj && !isNaN(dueObj.getTime()) ? dueObj.toLocaleDateString("en-IN", {
      day: "2-digit",
      month: "2-digit",
      year: "numeric"
    }) : member.membershipDue || "N/A";
    const message = `\u{1F4AA} *FITNESS WORLD* \u{1F9FE}
Unisex Gym & Fitness Center

Dear *${member.name}*,
Thank you for your payment! Here is your payment receipt:

\u{1F9FE} *Receipt No:* ${receiptNo}
\u{1F4C5} *Date:* ${dateFormatted}
\u{1F4B0} *Amount Paid:* \u20B9${amount}
\u{1F4B3} *Method:* ${method}
\u{1F4C5} *Membership Due:* ${dueFormatted}
\u{1F4B5} *Pending Balance:* \u20B9${balance}
${note ? `\u{1F4DD} *Note:* ${note}
` : ""}
Thank you for training with us! Keep up the hard work! \u{1F4AA}\u{1F525}`;
...
__name(sendReceiptWhatsApp, "sendReceiptWhatsApp");
...
__name(supabaseJson, "supabaseJson");
...
__name(supabaseAuthAdminJson, "supabaseAuthAdminJson");
...
__name(supabaseRaw, "supabaseRaw");
...
__name(supabaseServiceKey, "supabaseServiceKey");
...
__name(supabasePublishableKey, "supabasePublishableKey");
function validateMemberInput(body) {
  const input = {
    ...
    partialPaidAmount: body.partialPaidAmount !== void 0 || body.partial_paid_amount !== void 0 ? requiredNumber(body.partialPaidAmount ?? body.partial_paid_amount, "partial paid amount", 0) : 0,
    balanceAmount: body.balanceAmount !== void 0 || body.balance_amount !== void 0 ? requiredNumber(body.balanceAmount ?? body.balance_amount, "balance amount", 0) : 0
  };
...
__name(validateMemberInput, "validateMemberInput");
function validateRenewInput(body) {
  const input = {
    membershipStart: requiredDate(body.membershipStart ?? body.membership_start, "membership start"),
    membershipDue: requiredDate(body.membershipDue ?? body.membership_due, "membership due"),
    feesAmount: requiredNumber(body.feesAmount ?? body.fees_amount, "fees amount", 0),
    planType: body.planType ?? body.plan_type ? assertEnum(body.planType ?? body.plan_type, PLAN_TYPES, "plan type") : void 0
  };
  assertDueDateOrder(input.membershipStart, input.membershipDue);
  return input;
}
__name(validateRenewInput, "validateRenewInput");
function validatePaymentReceiptInput(body) {
  ...
  return {
    ...
    amount: requiredNumber(body.amount, "payment amount", 1, 1e6),
    method: assertEnum(body.method, PAYMENT_METHODS, "payment method"),
    note: note ?? "",
    receiptNo
  };
}
__name(validatePaymentReceiptInput, "validatePaymentReceiptInput");
...
__name(assertDueDateOrder, "assertDueDateOrder");
...
__name(calculatePlanDueDate, "calculatePlanDueDate");
...
__name(addCalendarMonthsIso, "addCalendarMonthsIso");
function daysInUtcMonth(year, monthIndex) {
  return new Date(Date.UTC(year, monthIndex + 1, 0)).getUTCDate();
}
__name(daysInUtcMonth, "daysInUtcMonth");
function validateAttendanceInput(body) {
  return {
    ...
    weightKg: body.weightKg === void 0 && body.weight_kg === void 0 ? void 0 : requiredNumber(body.weightKg ?? body.weight_kg, "weight", 20, 250)
  };
}
__name(validateAttendanceInput, "validateAttendanceInput");
...
__name(validateSmsInput, "validateSmsInput");
function validateSmsMemberInput(body) {
  return {
    memberId: assertUuid(body.memberId ?? body.member_id, "member id")
  };
}
__name(validateSmsMemberInput, "validateSmsMemberInput");
function generateReceiptNo(date = /* @__PURE__ */ new Date()) {
  const stamp = date.toISOString().replace(/[-:T.Z]/g, "").slice(0, 14);
  const suffix = Math.random().toString(36).slice(2, 6).toUpperCase();
  return `FW-R-${stamp}-${suffix}`;
}
__name(generateReceiptNo, "generateReceiptNo");
...
__name(paymentStatusFromAmounts, "paymentStatusFromAmounts");
...
__name(memberInputToDb, "memberInputToDb");
...
__name(paymentReceiptInputToDb, "paymentReceiptInputToDb");
function mapMember(row) {
  return {
    ...
    bmi: Number(row.bmi),
    goal: row.goal,
    goalOther: row.goal_other ?? void 0,
    healthProblem: row.health_problem ?? void 0,
    specialInstruction: row.special_instruction ?? void 0,
    warmupExercises: row.warmup_exercises ?? void 0,
    flexibilityTraining: row.flexibility_training ?? void 0,
    cardioTraining: row.cardio_training ?? void 0,
...
__name(mapPaymentReceipt, "mapPaymentReceipt");
...
__name(mapRenewalHistory, "mapRenewalHistory");
function mapAttendance(row) {
  return {
    id: row.id,
    memberId: row.member_id,
    visitDate: row.visit_date,
    weightKg: row.weight_kg ?? void 0,
    createdAt: row.created_at
  };
}
__name(mapAttendance, "mapAttendance");
...
__name(requiredDate, "requiredDate");
...
__name(isRealIsoDate, "isRealIsoDate");
...
__name(toDbColumn, "toDbColumn");
function addDays(date, days) {
  return new Date(date.getTime() + days * ONE_DAY_MS);
}
__name(addDays, "addDays");
function isoDateInTimeZone(date = /* @__PURE__ */ new Date()) {
...
__name(isoDateInTimeZone, "isoDateInTimeZone");
...
// .wrangler/tmp/bundle-h9usQW/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-h9usQW/middleware-loader.entry.ts
